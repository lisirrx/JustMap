...
public abstract class AbstractMiniMapRenderer {
	...
	protected abstract void render(DrawContext drawContext, double scale);
	...
	public void renderMap(DrawContext drawContext) {
		...
		this.render(drawContext, scale);

		if (mapSkin != null) {
			int skinX = minimap.getSkinX();
			int skinY = minimap.getSkinY();
			int brd = minimap.getBorder() * 2;
			this.mapSkin.draw(drawContext, skinX, skinY, mapWidth + brd, mapHeight + brd);
		}

		RenderUtil.drawRightAlignedString(
				drawContext, Double.toString(1 / mapScale),
		...
		if (ClientSettings.arrowIconType == ArrowType.DIRECTION_ARROW) {
			...
			MapPlayerManager.getPlayer(minecraft.player).getIcon().draw(drawContext, centerX, centerY, iconSize, true);
		}
		textManager.draw(drawContext);
